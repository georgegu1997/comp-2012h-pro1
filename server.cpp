...
{
  int n;
  struct package pkg = gen_pkg(0);
  if (pkg.gen_time - *last_change >= change_interval) {
    change_price();
  }

  n = write(newsockfd, &pkg, sizeof(pkg));
  if (n<0) error("ERROE writing to socket");

  printf("$%0.2f at %s", pkg.gen_price, ctime(&(pkg.gen_time)));

  sleep(gen_interval);
}

struct Server::package Server::gen_pkg(int arg1)
{
...
struct Server::package Server::gen_response_pkg(int arg1, struct package arg2)
{
  struct package new_pkg;
  new_pkg.type = arg1;
  new_pkg.gen_time = time(NULL);
...
void Server::process_buy_request(int newsockfd)
{
  ...
  if (rcv_pkg.type == 1) {
    *buy_count += 1;
    ...
    printf("Receiving wrong package\n");
  }
}
